...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " nix-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  nix-tx [options] <hex-tx> [commands]  Update hex-encoded nix transaction\n" +
            "or:     nix-tx [options] -create [commands]   Create hex-encoded nix transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded nix transaction
